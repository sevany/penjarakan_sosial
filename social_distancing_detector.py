# imports
from pengaturan.mailer import Mailer
from pengaturan import config
...
while True:
    ...
    text1 = "Jarak Selamat: > 2 m".format(config.MIN_DISTANCE)
    cv2.putText(frame, text1, (470, frame.shape[0] - 25), cv2.FONT_HERSHEY_SIMPLEX, 0.60, (255, 0, 0), 2)

    # draw the total number of social distancing violations on the output frame
    text = "Jumlah Berisiko: {}".format(len(serious))
    # print(len(serious) / config.People_Counter)
    cv2.putText(frame, text, (10, frame.shape[0] - 25), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 3)
    

    Threshold = "Limit Orang: {}".format(config.Threshold)
    cv2.putText(frame, Threshold, (470, frame.shape[0] - 50), cv2.FONT_HERSHEY_SIMPLEX, 0.60, (255, 0, 0), 2)
    
 #---------------------------------------------Alert function------------------------------------------------#   
    if len(serious) >= config.Threshold:
        cv2.putText(frame, "Amaran: Penjarakan Sosial", (10, frame.shape[0] - 80),
